...
//                }, 0);
            ...
            sortData = function (index, idxcol, reload, sor, obj) {
                ...
                if (ts.p.multiSort) {
                    ...
                    $("span.ui-grid-ico-sort", previousSelectedTh).addClass('ui-state-disabled');
                     $("span.ui-icon-both",previousSelectedTh).removeClass('ui-state-disabled');
                    ...
                    $("span.ui-icon-both",newSelectedTh).addClass("ui-state-disabled");
            ...
            imgs = "<span class='s-ico' style='display:inline'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc" + iac + " ui-state-disabled ui-sort-" + dir + "'></span>";
            // modify by lay
            //imgs += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc" + idc + " ui-state-disabled ui-sort-" + dir + "'></span></span>";
            imgs += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc" + idc + " ui-state-disabled ui-sort-" + dir + "'></span>";
            var appendImgs = "<span  class='ui-grid-ico-sort ui-icon-both'></span></span>";
            //end
            if (ts.p.multiSort) {
            ...
            for (i = 0; i < this.p.colNames.length; i++) {
                ...
                thead += "<div style='vertical-align:middle; text-align:" + align + "' id='jqgh_" + ts.p.id + "_" + ts.p.colModel[i].name + "' " + tdc + ">" 
                  + "<span>"+ts.p.colNames[i]+"</span>";
                ...
                thead += imgs+(ts.p.colModel[i].sortable===false?"</span>":appendImgs) + "</div></th>";
            }
            thead += "</tr></thead>";
            imgs = null;
            $(this).append(thead);
        ...
        return this.each(function () {
            ...
            */
            ...
            this.createTableRowForRule = function (rule, group) {
                ...
                // populate drop down with system provided column definitions
                var j = 0;
                for (i = 0; i < that.p.columns.length; i++) {
                    // but show only serchable and serchhidden = true fields
                    var searchable = (that.p.columns[i].search === undefined) ? true : that.p.columns[i].search,
                    ...
                    if (!newObj) {
                        ...
                        if (ylen >= 1) {
                            ...
                            for (k = 0; k < ylen; k++) {
                                ...
                                // Check to see if we have system defined conditions
